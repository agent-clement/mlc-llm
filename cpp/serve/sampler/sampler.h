...
class SamplerObj : public Object {
 public:
  ...
  /*! \brief Return whether the sampler can return sampled token ids without CPU sync. */
  virtual bool SupportsDeviceTokenIds() { return false; }

  /*!
   * \brief Sample token ids from a top-p-renormalized probability tensor and keep results on device.
   * \return A device tensor of shape (num_samples,) and dtype int32, or null if unsupported.
   */
  virtual Tensor BatchSampleTokenIdsDeviceAfterTopP(
      Tensor probs,                                   //
      const std::vector<int>& sample_indices,         //
      const Array<String>& request_ids,               //
      const Array<GenerationConfig>& generation_cfg,  //
